function man --wraps man --description 'Format and display manual pages'
    set -q man_blink; and set -l blink (set_color $man_blink); or set -l blink (set_color -o magenta)
    set -q man_bold; and set -l bold (set_color $man_bold); or set -l bold (set_color -o brred)
    set -q man_standout; and set -l standout (set_color $man_standout); or set -l standout (set_color -b yellow; set_color black)
    set -q man_underline; and set -l underline (set_color $man_underline); or set -l underline (set_color -uo brgreen)
    ...
    if test -d "$fish_manpath" -a -n "$MANPATH"
        ...
        env MANWIDTH=$width command man $argv
        return
    end
