...
. ${PROJ_HOME}/script/helpers/spinner/spinner.sh

[[ ! -z ${CONFIG+x} ]] && eval $(${PROJ_HOME}/script/helpers/parser_yaml ${CONFIG} "CONFIG_")
...
setup_func() {
  if [ -d "${ZDOTDIR:-$HOME}/.zprezto" ]; then
    ...
    echo "[1;36m[+][0m It seems that [1m[4mprezto[0m was installed before. Uninstall and reinstall it"
  fi
  [[ ${VERBOSE} == "true" ]] \
    && echo "[1;36m[+][0m Installing [1m[4mprezto[0m..." \
...
if [[ ! -z ${CONFIG+x} ]]; then
  if [[ ${CONFIG_prezto_install} == "true" ]]; then
    ...
    echo "[1;36m[+][0m [1m[4mprezto[0m is not installed"
  fi
else
  while true; do
    read "yn?[1;33m[?][0m Do you wish to install [1m[4mprezto[0m? "
    # vared -p "Do you wish to install prezto? " -c yn
    case $yn in
      [Yy]* ) echo "[1;36m[+][0m Install [1m[4mprezto[0m"; setup_func; break;;
      [Nn]* ) echo "[1;36m[+][0m Aborting install [1m[4mprezto[0m"; break;;
      * ) echo "[1;31m[!][0mPlease answer yes or no";;
    esac
  done
fi
