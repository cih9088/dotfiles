...
setup_func_local() {
  ...
  if [[ "remove update"  == *"${COMMAND}"* ]]; then
    if [ -d ${PREFIX}/src/libX11-* ]; then
      ++ pushd ${PREFIX}/src/libX11-*
      make uninstall || true
      make clean || true
      ++ popd
      rm -rf ${PREFIX}/src/libX11-*
    else
  ...
  case "${PLATFORM}" in
    ...
    LINUX)
      case "${FAMILY}" in
        DEBIAN)
          if [ "${COMMAND}" == "remove" ]; then
            ++ sudo apt-get -y remove libx11-dev
          elif [ "${COMMAND}" == "install" ]; then
            ++ sudo apt-get -y install libx11-dev
          elif [ "${COMMAND}" == "update" ]; then
            ++ sudo apt-get -y --only-upgrade install libx11-dev
          fi
          ;;
        RHEL)
          if [ "${COMMAND}" == "remove" ]; then
            ++ sudo dnf -y remove libX11-devel
          elif [ "${COMMAND}" == "install" ]; then
            ++ sudo dnf -y install libX11-devel
          elif [ "${COMMAND}" == "update" ]; then
            ++ sudo dnf -y update libX11-devel
          fi
          ;;
      esac
      ;;
