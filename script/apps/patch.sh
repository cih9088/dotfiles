...
setup_func_local() {
  ...
  if [[ "install update"  == *"${COMMAND}"* ]]; then
    if [ -z "${SRC_PATH}" ]; then
      ...
      ++ curl -LO "https://ftp.gnu.org/gnu/patch/patch-${VERSION}.tar.gz"
      ++ tar -xvzf "patch-${VERSION}.tar.gz"
  ...
  case "${PLATFORM}" in
    ...
    LINUX)
      case "${FAMILY}" in
        DEBIAN)
          if [ "${COMMAND}" == "remove" ]; then
            ++ sudo apt-get -y remove patch
          elif [ "${COMMAND}" == "install" ]; then
            ++ sudo apt-get -y install patch
          elif [ "${COMMAND}" == "update" ]; then
            ++ sudo apt-get -y --only-upgrade install patch
          fi
          ;;
        RHEL)
          if [ "${COMMAND}" == "remove" ]; then
            ++ sudo dnf -y remove patch
          elif [ "${COMMAND}" == "install" ]; then
            ++ sudo dnf -y install patch
          elif [ "${COMMAND}" == "update" ]; then
            ++ sudo dnf -y update patch
          fi
          ;;
      esac
      ;;
