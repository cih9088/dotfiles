...
setup_func_local() {
  ...
  if [ -d $HOME/.local/src/pkg-config-* ]; then
    if [ ${FORCE} == 'true' ]; then
      pushd $HOME/.local/src/pkg-config-*
      make uninstall || true
      make clean || true
      popd
      rm -rf $HOME/.local/src/pkg-config-*
      DO_INSTALL=true
    fi
  else
    DO_INSTALL=true
  ...
  if [ ${DO_INSTALL} == 'true' ]; then
    ...
    mv pkg-config-${VERSION} $HOME/.local/src
    pushd $HOME/.local/src/pkg-config-${VERSION}

    ./configure --prefix=$HOME/.local --with-internal-glib || exit $?
    make || exit $?
    make install || exit $?
